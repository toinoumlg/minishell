...
/*   Updated: 2025/09/23 18:39:07 by amalangu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	check_valid_redirects(t_token *tokens)
{
	while (tokens)
	{
...
static int	check_for_valid_pipe(t_token *tokens)
{
	int	is_empty;
...
int	add_operator(char **parse_error, t_minishell *minishell)
{
	...
	if (!new_token->type)
		return (1);
	if (check_for_valid_pipe(minishell->tokens)
		|| check_valid_redirects(minishell->tokens))
		return (1);
